...
struct Bar
{
   ...
   std::string CONTENT_CHARACTER        = BarSettings::DEFAULT_CONTENT_CHARACTER;
   std::string SPACER_CHARACTER         = BarSettings::DEFAULT_SPACER_CHARACTER;
   std::string END_CHARACTER            = BarSettings::DEFAULT_END_CHARACTER;
   std::string EOL_CHARACTER            = BarSettings::DEFAULT_EOL_CHARACTER;
...
namespace BarMan
{
   int Init(Bar* bar)
   {
      ...
      bar->SPACER_CHARACTER                = (bar->CUSTOM_SPACER_CHARACTER == "")  ? BarSettings::DEFAULT_SPACER_CHARACTER  : bar->CUSTOM_SPACER_CHARACTER;    // Configure SPACER_CHARACTER
      bar->END_CHARACTER                   = (bar->CUSTOM_END_CHARACTER == "")     ? BarSettings::DEFAULT_END_CHARACTER     : bar->CUSTOM_END_CHARACTER;       // Configure END_CHARACTER
      bar->EOL_CHARACTER                   = (bar->CUSTOM_EOL_CHARACTER == "")     ? BarSettings::DEFAULT_EOL_CHARACTER     : bar->CUSTOM_EOL_CHARACTER;       // Configure EOL_CHARACTER
      return 0;
   }
...
namespace BarUtils 
{
   // Fill this out and pass it to the Create utility to easily create a bar.
   struct BarInitForm
   {
      std::string title;
      int startProgress;
      ...
      int CUSTOM_TITLE_LENGTH              = 0;                     // To nullify, set to 0
      std::string CUSTOM_CONTENT_CHARACTER = "";   // To nullify, set to ""
      std::string CUSTOM_SPACER_CHARACTER  = "";   // To nullify, set to ""
      std::string CUSTOM_END_CHARACTER     = "";   // To nullify, set to ""
      std::string CUSTOM_EOL_CHARACTER     = "";   // To nullify, set to ""
   ...
   Bar* Create(std::string title, int startProgress = 0)
   {
      Bar* bar                             = new Bar;
      bar->TITLE                           = title;
      bar->PROGRESS                        = startProgress;
      return bar;
   }

   Bar* Create(BarInitForm form)
   {
      Bar* bar                             = new Bar;
      ...
      bar->MAX_TITLE_LENGTH                = (form.CUSTOM_TITLE_LENGTH == 0)       ? BarSettings::DEFAULT_TITLE_LENGTH      : form.CUSTOM_TITLE_LENGTH;
      bar->CONTENT_CHARACTER               = (form.CUSTOM_CONTENT_CHARACTER == "") ? BarSettings::DEFAULT_CONTENT_CHARACTER : form.CUSTOM_CONTENT_CHARACTER;
      bar->SPACER_CHARACTER                = (form.CUSTOM_SPACER_CHARACTER == "")  ? BarSettings::DEFAULT_SPACER_CHARACTER  : form.CUSTOM_SPACER_CHARACTER;
      bar->END_CHARACTER                   = (form.CUSTOM_END_CHARACTER == "")     ? BarSettings::DEFAULT_END_CHARACTER     : form.CUSTOM_END_CHARACTER;
      bar->EOL_CHARACTER                   = (form.CUSTOM_EOL_CHARACTER == "")     ? BarSettings::DEFAULT_EOL_CHARACTER     : form.CUSTOM_EOL_CHARACTER;
      return bar;
   }
   ...
   void Increment(Bar* bar, int incrementLevel = 1)
   {
      bar->PROGRESS += incrementLevel;
   }

   // Decrement the bar's progress by decrementLevel.
   void Decrement(Bar* bar, int decrementLevel = 1)
   {
   ...
   void Update(Bar* bar)
   {
      ...
      if (bar->TRUNCATE_TITLES)
      {
         ...
         if (bar->TITLE.length() < bar->MAX_TITLE_LENGTH)
         {
            ...
            while (i < o)
            {
               bar->TITLE += bar->SPACER_CHARACTER;
               i++;
            }
         }
      }
